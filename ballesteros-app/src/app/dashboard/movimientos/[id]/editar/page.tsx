...
import { ArrowLeft, Save, DollarSign, TrendingUp, TrendingDown, Trash2 } from 'lucide-react'
...
export default function EditarMovimientoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const cargarMovimiento = async (id: number) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('Error al cargar movimiento')
      }
      ...
      setFormData({
        ...
        es_ingreso: movimiento.es_ingreso,
        monto: movimiento.monto?.toString() || '',
        fecha: movimiento.fecha ? new Date(movimiento.fecha).toISOString().slice(0, 16) : '',
        empresa_id: movimiento.empresa_id?.toString() || '',
        corte_id: movimiento.corte_id?.toString() || '',
        cuenta_origen_id: movimiento.cuenta_origen_id?.toString() || '',
        cuenta_destino_id: movimiento.cuenta_destino_id?.toString() || '',
        entidad_relacionada_id: movimiento.entidad_relacionada_id?.toString() || '',
        empleado_responsable_id: movimiento.empleado_responsable_id?.toString() || '',
        categoria_id: movimiento.categoria_id?.toString() || '',
        subcategoria_id: movimiento.subcategoria_id?.toString() || '',
        forma_pago: movimiento.forma_pago || '',
        plataforma: movimiento.plataforma || '',
        referencia: movimiento.referencia || '',
        beneficiario: movimiento.beneficiario || '',
        comision: movimiento.comision?.toString() || '0'
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dataToSend = {
        ...
        corte_id: formData.corte_id ? parseInt(formData.corte_id) : null,
        cuenta_origen_id: formData.cuenta_origen_id ? parseInt(formData.cuenta_origen_id) : null,
        cuenta_destino_id: formData.cuenta_destino_id ? parseInt(formData.cuenta_destino_id) : null,
        entidad_relacionada_id: formData.entidad_relacionada_id ? parseInt(formData.entidad_relacionada_id) : null,
        empleado_responsable_id: formData.empleado_responsable_id ? parseInt(formData.empleado_responsable_id) : null,
        categoria_id: formData.categoria_id ? parseInt(formData.categoria_id) : null,
        subcategoria_id: formData.subcategoria_id ? parseInt(formData.subcategoria_id) : null,
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Editar Movimiento #{formData.id}</h1>
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Modifica el tipo de movimiento financiero
                </CardDescription>
              </CardHeader>
