...
export async function POST(request: NextRequest) {
  try {
    ...
    // Usar transacción para crear proveedor, asignarlo a empresas y crear saldo inicial
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      if (empresasActivas.length > 0) {
        ...
        // Si hay saldo inicial, crear saldos para todas las empresas (cuenta por pagar)
