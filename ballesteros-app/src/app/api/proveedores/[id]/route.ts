      ...
      if (saldo_inicial && saldo_inicial > 0) {
        ...
        if (!empresaParaSaldo) {
          ...
          if (carniceriaBallesteros) {
            ...
            console.log(`⚠️  Proveedor: No empresa activa especificada, usando Carnicería Ballesteros: ${carniceriaBallesteros.id}`) // TEMP DEBUG
          } else {
            ...
            if (empresasActivas.length > 0) {
              ...
              console.log(`⚠️  Proveedor: Carnicería Ballesteros no encontrada, usando fallback: ${empresasActivas[0].id}`) // TEMP DEBUG
            }
          }
        }
